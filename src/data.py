...
def preprocess_koi_dataframe(
    df: pd.DataFrame, *, return_identifiers: bool = False
) -> PreparedDataset:
    """Clean the KOI dataframe and return a balanced training view."""

    df = df.copy()

    identifier_frame = None
    identifier_cols = [col for col in IDENTIFIER_COLUMNS if col in df.columns]
    if return_identifiers and identifier_cols:
        identifier_frame = df[identifier_cols].copy()
    ...
    mask_valid = df[TARGET_COLUMN].isin(valid_labels)
    df = df[mask_valid].copy()
    if identifier_frame is not None:
        identifier_frame = identifier_frame.loc[mask_valid].copy()

    df.sort_values("koi_period", inplace=True)
    if identifier_frame is not None:
        identifier_frame = identifier_frame.loc[df.index].copy()
    ...
    numeric_mask = df[list(NUMERIC_FEATURE_COLUMNS)].notna().all(axis=1)
    df = df[numeric_mask].copy()
    if identifier_frame is not None:
        identifier_frame = identifier_frame.loc[numeric_mask].copy()
    ...
    if identifier_frame is not None:
        identifier_frame = identifier_frame.loc[df.index].reset_index(drop=True)
