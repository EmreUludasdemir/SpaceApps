...
- Hyperparameter optimisation: `GridSearchCV` evaluates a curated grid under 10-fold, 5-repeat stratified cross-validation and selects the configuration with the highest macro specificity (breaking ties with macro F1). Tuned parameters are cached so subsequent runs can skip the expensive search.
...
- Persisted artefacts: the tuned stacking pipeline, threshold wrapper, metrics (including per-fold diagnostics, validation/full-dataset confusion matrices, threshold sweep), feature lists, and a ranked table of the highest-confidence candidate predictions are saved under `models/`.
...
2. **Train and optimise the model** (downloads the latest KOI data and saves artefacts under `models/`):
   ...
   - `--no-reuse-tuning` — force a fresh GridSearchCV run even if cached tuned parameters exist in `metrics.json`.
   - `--cv-jobs -1` — control how many parallel workers handle the repeated cross-validation (default uses all available cores).
...
- **Full dataset audit** – `full_dataset` stores threshold-adjusted metrics and specificity values on the complete KOI catalogue (without down-sampling) to verify how the tuned model behaves on the imbalanced data distribution.
- **Top candidate list** – `top_predictions` captures the highest-probability KOIs according to the tuned model (with identifiers and feature snapshots) so you can immediately review the most confident candidate predictions offline or in the Streamlit UI.
